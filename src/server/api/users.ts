...
const asyncMiddleware = (fn: (req: any, res: any) => Promise<void>) => {
  return (req, res, next) => {
...
router.post('/deactivate', asyncMiddleware(async (req, res) => {
  if (!isObjectRecord(req.body)) {
...
router.post('/password', asyncMiddleware(async (req, res) => {
  if (!isObjectRecord(req.body)) {
